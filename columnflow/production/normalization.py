...
def normalization_weights_setup(
    ...
    normalization_selection_stats_avg = {
        ...
        for dataset, dataset_dict in selection_stats.items()
    }
    ...
    def stitching_weight_cclub():
        ...
        for process_inst in process_insts:
            try:
                ...
                incl_sum_per_process = selection_stats[self.inclusive_dataset.name]["sum_mc_weight_per_process"]
                incl_sum = selection_stats[self.inclusive_dataset.name]["sum_mc_weight"]
                nominator = incl_sum_per_process[process_id]/incl_sum
                denominator = np.sum([
                    (
                        selection_stats[x]["sum_mc_weight_per_process"].get(process_id, 0)
                        / selection_stats[x]["sum_mc_weight"]
                        * selection_stats[x]["num_events"]
                    )
                    for x in selection_stats.keys() 
                ])
            ...
            process_weight_table[0, process_inst.id] = (
                ...
                # / ction_stats_avg[self.dataset_inst.name]["sum_mc_weight"]
            )
